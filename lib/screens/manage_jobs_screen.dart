...
class ManageJobsScreen extends StatefulWidget {
  final int level;
  const ManageJobsScreen({super.key, required this.level});
  @override
...
class _ManageJobsScreenState extends State<ManageJobsScreen> {
  ...
  Future<void> _endJob(Map j) async {
    ...
    final finalShotsCtrl =
        TextEditingController(text: '${j['shotsCompleted'] ?? 0}');
    ...
    final finalShots =
        int.tryParse(finalShotsCtrl.text.trim()) ?? (j['shotsCompleted'] ?? 0);
        ...
        final selectedJob = await showDialog<Map?>(
          ...
          builder: (context) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                Text(
                    '${j['productName']} completed with ${j['shotsCompleted'] ?? 0} shots.'),
                const SizedBox(height: 16),
                const Text('Select next job to start:',
                    style: TextStyle(fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                ...queuedJobs.map((job) => ListTile(
                      title: Text(job!['productName'] ?? 'Unknown'),
                      subtitle: Text(
                          '${job['color'] ?? ''} • Target: ${job['targetShots'] ?? 0}'),
              ...
              builder: (context, setDialogState) => AlertDialog(
                ...
                content: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  TextField(
                      ...
                      keyboardType: TextInputType.number,
                      decoration:
                          const InputDecoration(labelText: 'Target Shots')),
                  const SizedBox(height: 8),
                  ...
                  const SizedBox(height: 8),
                  const Text(
                      'Note: Assign this job to a machine in Production Planning',
  ...
  void _edit(Map j) async {
    final productCtrl =
        TextEditingController(text: j['productName'] as String?);
              ...
              delegate: SliverChildBuilderDelegate(
                (_, i) {
                  ...
                  if (j['shotsCompleted'] != null && j['targetShots'] != null) {
                    print(
                        'DEBUG Job ${j['productName']}: status=$status, shots=${j['shotsCompleted']}/${j['targetShots']}');
                  }
