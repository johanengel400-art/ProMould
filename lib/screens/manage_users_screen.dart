...
class ManageUsersScreen extends StatefulWidget {
  final int level;
  const ManageUsersScreen({super.key, required this.level});
  @override
...
class _ManageUsersScreenState extends State<ManageUsersScreen> {
              ...
              builder: (context, setDialogState) => AlertDialog(
                ...
                content: SingleChildScrollView(
                  ...
                  DropdownButtonFormField<String>(
                      value: shift,
                      items: const ['Day', 'Night', 'Any']
                          .map(
                              (s) => DropdownMenuItem(value: s, child: Text(s)))
                          .toList(),
                      onChanged: (v) =>
                          setDialogState(() => shift = v ?? 'Day'),
                      decoration: const InputDecoration(labelText: 'Shift')),
                  const SizedBox(height: 8),
                  if (level == 1) ...[
                    const Divider(),
                    const Text('Operator Machine Assignment',
                    ...
                    DropdownButtonFormField<String?>(
                      ...
                      items: [
                        const DropdownMenuItem(
                            value: null,
                            child: Text('No Assignment (See All)')),
                        ...machines.map((m) => DropdownMenuItem(
                            value: m['id'] as String,
                            child: Text(m['name'] as String))),
                      ],
                      onChanged: (v) =>
                          setDialogState(() => assignedMachineId = v),
                  ...
                  if (level >= 2 && level <= 3) ...[
                    const Divider(),
                    const Text('Setter/Material Floor Assignment',
                        style: TextStyle(fontWeight: FontWeight.bold)),
                    const SizedBox(height: 8),
                    DropdownButtonFormField<String?>(
                      ...
                      items: [
                        const DropdownMenuItem(
                            value: null,
                            child: Text('No Assignment (All Floors)')),
                        ...floors.map((f) => DropdownMenuItem(
                            value: f['id'] as String,
                            child: Text(f['name'] as String))),
                      ],
                      onChanged: (v) =>
                          setDialogState(() => assignedFloorId = v),
                      decoration: const InputDecoration(
                  ...
                  ElevatedButton(
                      onPressed: () async {
                        ...
                        final data = {
                          ...
                          'shift': shift,
                          'assignedMachineId':
                              level == 1 ? assignedMachineId : null,
                          'assignedFloorId': (level >= 2 && level <= 3)
                              ? assignedFloorId
                              : null,
                        };
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Row(
                children: [
                  Expanded(
                      child: _buildStatCard('Operators', operators.toString(),
                          const Color(0xFF4CC9F0))),
                  const SizedBox(width: 8),
                  Expanded(
                      child: _buildStatCard('Material', material.toString(),
                          const Color(0xFF06D6A0))),
                  const SizedBox(width: 8),
                  Expanded(
                      child: _buildStatCard('Setters', setters.toString(),
                          const Color(0xFFFFD166))),
                  const SizedBox(width: 8),
                  Expanded(
                      child: _buildStatCard('Mgmt', management.toString(),
                ...
                (_, i) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                            ...
                            Row(
                              children: [
                                ...
                                Text(
                                  ...
                                  style: const TextStyle(
                                      color: Color(0xFF4CC9F0), fontSize: 11),
                                ),
                              ],
                            ),
                          ],
                          if ((u['level'] == 2 || u['level'] == 3) &&
                              u['assignedFloorId'] != null) ...[
                            const SizedBox(height: 6),
                            Row(
                              children: [
                      ...
                      onTap: () =>
                          _addOrEdit(user: Map<String, dynamic>.from(u)),
                      trailing: IconButton(
  ...
  Color _getLevelColor(int level) {
    switch (level) {
      case 1:
        return const Color(0xFF4CC9F0); // Operator - Cyan
      case 2:
        return const Color(0xFF06D6A0); // Material - Green
      case 3:
        return const Color(0xFFFFD166); // Setter - Yellow
      case 4:
        return const Color(0xFF9D4EDD); // Management - Purple
      default:
