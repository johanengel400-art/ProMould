...
class NotificationService {
    ...
    for (final job in runningJobs) {
      ...
      if (minutesRemaining <= 30 && minutesRemaining > 0) {
        ...
        _notifications.add({
          ...
          'message':
              '${job['productName']} on ${machine?['name'] ?? 'Unknown'} will finish in ${minutesRemaining.round()} min',
    ...
    for (final machine in machinesBox.values.cast<Map>()) {
      ...
      if (scrapRate > 5.0) {
        _notifications.add({
          ...
          'message':
              '${machine['name']}: ${scrapRate.toStringAsFixed(1)}% scrap rate',
          'timestamp': DateTime.now(),
          'data': machine,
        });
      }
  ...
  static void _checkMaintenanceAlerts() {
    ...
    for (final machine in machinesBox.values.cast<Map>()) {
      ...
      final maintenanceInterval =
          machine['maintenanceInterval'] as int? ?? 100000; // shots

      // Calculate total shots since last maintenance
      final inputs = inputsBox.values
          .cast<Map>()
          .where((i) => i['machineId'] == machineId);
      ...
      for (final input in inputs) {
        ...
        if (lastMaintenanceDate == null ||
            (inputDate != null && inputDate.isAfter(lastMaintenanceDate))) {
      ...
      if (shotsSinceMaintenance >= maintenanceInterval * 0.9) {
        _notifications.add({
          ...
          'priority':
              shotsSinceMaintenance >= maintenanceInterval ? 'high' : 'medium',
          'icon': Icons.build,
          'color': const Color(0xFF4CC9F0),
          'title': 'Maintenance Due',
          'message':
              '${machine['name']}: $shotsSinceMaintenance shots since last maintenance',
          'timestamp': DateTime.now(),
          'data': machine,
        });
      }
    ...
    for (final change in mouldChangesBox.values.cast<Map>()) {
      ...
      if (minutesUntil > 0 && minutesUntil <= 60) {
        ...
        _notifications.add({
          ...
          'message':
              '${machine?['name'] ?? 'Machine'} mould change in $minutesUntil min',
          'timestamp': DateTime.now(),
          'data': change,
        });
      }
      ...
      if (minutesUntil < 0) {
        ...
        _notifications.add({
          ...
          'message':
              '${machine?['name'] ?? 'Machine'} mould change is ${(-minutesUntil)} min overdue',
          'timestamp': DateTime.now(),
          'data': change,
        });
      }
