...
class OverrunNotificationService {
  ...
  static void start() {
    ...
    LogService.service(
        'OverrunNotificationService', 'Starting overrun monitoring...');
  ...
  static Future<void> _checkOverruns() async {
    try {
      ...
      _lastNotified.removeWhere(
          (jobId, _) => !overrunningJobs.any((j) => j['id'] == jobId));
      _notificationCount.removeWhere(
          (jobId, _) => !overrunningJobs.any((j) => j['id'] == jobId));
    } catch (e) {
  ...
  static Future<void> _sendOverrunNotification(
    ...
    try {
      ...
      final durationFormatted =
          JobStatus.formatOverrunDuration(durationMinutes);
      ...
      switch (level) {
        case 'CRITICAL':
          ...
          body =
              '$productName on $machineId has been overrunning for $durationFormatted! '
        ...
        case 'HIGH':
          ...
          body =
              '$productName on $machineId overrunning for $durationFormatted. '
        ...
        default:
          ...
          body =
              '$productName on $machineId has exceeded target by $overrunShots shots '
  ...
  static Map<String, dynamic>? getJobNotificationInfo(String jobId) {
    ...
    return {
      ...
      'minutesSinceLastNotification':
          DateTime.now().difference(lastNotification).inMinutes,
    };
  }
}
